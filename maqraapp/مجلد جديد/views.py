...
from .forms import StudentForm, TeacherForm, LoginForm, DailyMemorizationTaskForm, DailyReviewTaskForm, DailyTalqeenTaskForm, WeeklyLectureForm, WeeklyPlanForm, DepartmentForm, MsarForm, HalaqaForm
from .models import Students, Teachers, Halaqa, UserRole, Supervisor, User, DailyMemorizationTask, DailyReviewTask, DailyTalqeenTask, WeeklyLecture, WeeklyPlan, Department, Msar, Language
...
#
 #               # تحقق مما إذا كان المستخدم طالبًا أو معلمًا
 #               if user.groups.filter(name='students').exists():
 #                   return redirect('student_home')  # توجيه الطلاب إلى الصفحة الرئيسية للطلاب
  #              elif user.groups.filter(name='teachers').exists():
  #                  return redirect('teacher_home')  # توجيه المعلمين إلى الصفحة الرئيسية للمعلمين
 #               else:
  #                  messages.error(request, "حسابك ليس مرتبطًا بأي مجموعة.")
  #                  return redirect('login')
  #          else:
...
#            else:
#                messages.error(request, "ليس لديك صلاحية الوصول كطالب.")
 #               return redirect('login')
 #       else:
 #           messages.error(request, "اسم المستخدم أو كلمة المرور غير صحيحة")
 #           return render(request, 'index.html', {'error': 'اسم المستخدم أو كلمة المرور غير صحيحة'})
#
...
#    classes = ClassQ.objects.filter(teacher=teacher)
#    return render(request, 'teacher/teacher_home.html', {'classes': classes})
...
def statistics_dashboard_view(request):
    ...
    for msar in masarat_stats:
        masarat_chart_data.append({
            'name': msar.name,
            'student_count': msar.student_count if msar.student_count is not None else 0
        })
    ...
    for halaqa in halaqat_stats:
        halaqat_chart_data.append({
            'name': halaqa.name,
            'student_count': halaqa.student_count if halaqa.student_count is not None else 0
        })
    ...
    for msar in hifz_stats_by_msar:
        hifz_chart_data.append({
            'name': msar.name,
            'total_pages_and_parts': msar.total_pages_and_parts if msar.total_pages_and_parts is not None else 0
        })
...
def export_student_report_excel(request):
    ...
    for student in students:
        worksheet.append([
            ...
            student.whatsapp, # Include whatsapp number
        ])
...
def save_memorization_task(request):
    if request.method == "POST":
        ...
        task = DailyMemorizationTask(
            ...
            MemorizationTask=data["category"],
        ...
        task.save()
        return JsonResponse({"message": "تم حفظ المهمة بنجاح!"})
    return JsonResponse({"message": "فشل في حفظ المهمة."}, status=400)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.user == student_user:
                message.receiver = teacher.user
            else:
                message.receiver = student_user
