...
function updateAyatOptions(surahSelect, rowIndex) {
    ...
    // إزالة عرض عدد الآيات للسورة
    const ayatCountDisplay = document.getElementById(`ayat-count-display-${rowIndex}`);
    if (ayatCountDisplay) {
        ayatCountDisplay.remove();
    }
...
function calculateTotalAyatMemorized() {
    ...
    const surahSelects = document.querySelectorAll("[id^='surah-options-']");
    const ayatSelects = document.querySelectorAll("[id^='ayat-options-']");

    for (let i = 0; i < surahSelects.length; i += 2) {
        const startSurah = surahSelects[i].value.trim();
        const startAyat = parseInt(ayatSelects[i].value, 10) || 0;
        const endSurah = surahSelects[i + 1].value.trim();
        const endAyat = parseInt(ayatSelects[i + 1].value, 10) || 0;

        if (startSurah === endSurah) {
            totalAyatMemorized += (endAyat - startAyat);
        } else {
            alert("قم بالادخال على مرحلتين، الحفظ يبدأ من سورة جديدة.");
            return;
        }
    }

    // عرض عدد الآيات المحفوظة للمستخدم في المكان المخصص
    const totalAyatDisplay = document.getElementById("total-ayat-memorized-cell");
    if (totalAyatDisplay) {
        totalAyatDisplay.textContent = `${totalAyatMemorized}`;
    }
}
...
// إضافة مستمعات التغيير لخيارات الآيات
document.querySelectorAll("[id^='ayat-options-']").forEach(ayatSelect => {
    ayatSelect.addEventListener('change', calculateTotalAyatMemorized);
});
