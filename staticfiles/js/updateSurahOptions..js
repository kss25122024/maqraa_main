document.addEventListener('DOMContentLoaded', function () {
    function updateSurahOptions(selectElement) {
        ...
        // تعبئة الخيارات الجديدة للآيات بناءً على السورة المختارة
        ...
        // قم بتحديث الخيارات عند تحميل الصفحة لأول مرة
        updateAyatOptions(previousSurahSelect, previousAyatSelect);
        updateAyatOptions(currentSurahSelect, currentAyatSelect);
    }

    const memorizationTaskField = document.querySelector('select[name="MemorizationTask"]');
    if (memorizationTaskField) {
        memorizationTaskField.addEventListener('change', function() {
            updateSurahOptions(this);
        });

        // قم بتحديث الخيارات عند تحميل الصفحة لأول مرة
        updateSurahOptions(memorizationTaskField);
    }
});
