document.addEventListener('DOMContentLoaded', function() {
    ...
    const surahField = document.getElementById('id_surah');
    const ayahField = document.getElementById('id_ayah_number');
    ...
    const ayahMapping = {
        ...
        "البقرة": 286,
        "آل عمران": 200,
        // باقي السور مع أرقام الآيات
    };

    categoryField.addEventListener('change', function() {
        const category = categoryField.value;
        surahField.innerHTML = '';
        if (surahMapping[category]) {
            surahMapping[category].forEach(function(surah) {
    ...
    surahField.addEventListener('change', function() {
        const surah = surahField.value;
        ayahField.innerHTML = '';
        if (ayahMapping[surah]) {
            for (let i = 1; i <= ayahMapping[surah]; i++) {
                const option = document.createElement('option');
                option.value = i;
                option.text = i;
                ayahField.add(option);
            }
        }
    });
});
